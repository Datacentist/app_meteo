...
try:
    # Nouvelle API (version 2.x) - minuscules
    from meteostat import Point, Stations, monthly
    Monthly = monthly
    METEOSTAT_VERSION = "2.x (lowercase)"
except ImportError:
    try:
        # Ancienne API (version 1.x) - majuscules
        from meteostat import Point, Monthly, Stations
        METEOSTAT_VERSION = "1.x (uppercase)"
    except ImportError:
        # Fallback avec import module
        try:
            import meteostat
            Point = meteostat.Point
            Stations = meteostat.Stations
            # Essayer les deux versions
            if hasattr(meteostat, 'monthly'):
                Monthly = meteostat.monthly
                METEOSTAT_VERSION = "2.x (module.monthly)"
            elif hasattr(meteostat, 'Monthly'):
                Monthly = meteostat.Monthly
                METEOSTAT_VERSION = "1.x (module.Monthly)"
            else:
                st.error("❌ Impossible de trouver la classe Monthly/monthly dans meteostat")
                st.stop()
        except Exception as e:
            st.error(f"❌ Erreur lors de l'import de meteostat: {str(e)}")
            st.stop()
...
def get_monthly_from_best_source(lat, lon, start, end):
    ...
    try:
        ...
        try:
            # Utiliser la classe Monthly (qui peut être monthly ou Monthly selon la version)
...
if st.button("🔮 Prédire le mois suivant (tavg)", type="primary"):
    with st.spinner("Récupération des données météorologiques..."):
        ...
        if debug:
            ...
            st.write(f"**Version API Meteostat:** {METEOSTAT_VERSION}")
...
with st.sidebar:
    ...
    st.divider()
    st.caption(f"Données: Meteostat | Modèle: LinearRegression")
    st.caption(f"API: {METEOSTAT_VERSION}")
